...
response = requests.get('https://www.geonames.org/postalcode-search.html?q=&country=FR')
...
for row in rows:
    ...
    if cols:
        ...
        values = cols[1:]
        table_dict[key] = dict(zip(['Place', 'Code', 'Country', 'State', 'City1', 'City2', 'City3'], values))
